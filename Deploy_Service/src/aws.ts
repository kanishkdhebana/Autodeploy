    ...
    if (data.Messages && data.Messages.length > 0) {
      ...
      for (const message of data.Messages) {
        ...
        if (message.ReceiptHandle) {
          ...
          console.warn(
            "Message is missing ReceiptHandle, cannot delete from queue.",
          );
        }
      }
...
async function downloadFileFromS3(fileName: string, localDir: string) {
  ...
  try {
    const data: GetObjectCommandOutput = await s3.send(
...
export async function copyFinalBuildToS3(id: string, buildFolder = "lib") {
  const __dirname = path.dirname(fileURLToPath(import.meta.url));
